...
export const Participate: FC<Props> = ({ event }) => {
  ...
  const isParticipator = useMemo(() => {
    return participators.some((par) => par.user._id === localStorage.getItem('uid'));
  }, [participators]);
    ...
    <Grid container justifyContent="space-evenly" alignItems="center">
      <Grid item sx={{ textAlign: 'center' }}>
        {isBefore(new Date(event.endDate), new Date()) ? (
          <>
            <Typography variant="h2">This event has ended</Typography>
            <Typography variant="h5">We hope it was great!</Typography>
          </>
        ) : (
          <>
            {isUserOwner ? (
