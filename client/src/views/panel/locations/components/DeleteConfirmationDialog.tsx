...
export const DeleteConfirmationDialog: React.FC<Props> = ({
  ...
  return (
    <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} TransitionComponent={DialogTransition}>
      <DialogTitle>Location removal confirmation</DialogTitle>
      <DialogContent>
        <Grid container justifyContent="center">
          {businessChain.locations.length === selectedLocations.length && (
            <Alert severity="warning" variant="filled">
              You are trying to remove all locations of your business. As a result, your whole business chain will be
              deleted.
            </Alert>
          )}
          <Typography sx={{ mt: 1 }}>
            You have selected <b>{selectedLocations.length}</b> locations to be removed. Please insert <b>delete</b> in
            the field below to confirm your decision.
          </Typography>
        </Grid>
        <Grid sx={{ mt: 2 }} container justifyContent="center">
          <TextField value={value} onChange={(e) => setValue(e.target.value)} label="delete" />
        </Grid>
      </DialogContent>
      <DialogActions>
        <Grid container justifyContent="flex-end">
          <LoadingButton loading={isLoading} disabled={isLoading || value !== 'delete'} onClick={handleClick}>
            Delete
          </LoadingButton>
        </Grid>
      </DialogActions>
