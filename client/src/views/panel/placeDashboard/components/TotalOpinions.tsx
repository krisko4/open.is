...
import { useGetOpinionsForSelectedLocationQuery } from 'store/api';
...
export const TotalOpinions: FC<any> = () => {
  ...
  const opinionsDiff = useMemo(() => {
    if (opinionData) {
      ...
      return (
        opinionData.opinions.length > 0 &&
        Math.round(
          ((opinionData.opinions.length / (opinionData.opinions.length - opinionData.today)) * 100 - 100) * 10
        ) / 10
      );
    }
    return 0;
